...
https://github.com/DrajatAkbarr/Bug-Bounty-Agent/raw/main/assets/assistant.mp4
...
**1. Skenario Golang: Uji Ketahanan Deteksi Anomali Jaringan** _Pilih dropdown: Go_ Prompt ini sangat cocok untuk memamerkan simulasi traffic jaringan tingkat rendah, seolah-olah mengaudit sistem deteksi DNS tunneling.
...
**2. Skenario Python: Audit Arsitektur Backend (Race Condition)** _Pilih dropdown: Python_ Mendemonstrasikan pengujian keamanan pada logic bisnis dashboard risk management.
...
**3. Skenario JavaScript: Eksploitasi Cloud Infrastructure (SSRF)** _Pilih dropdown: JS_ Skenario bug bounty modern untuk menembus metadata cloud (AWS/GCP) melalui backend Node.js.
...
**4. Skenario Bash: Local Privilege Escalation (Otomasi Server)** _Pilih dropdown: Bash_ Menunjukkan kemampuan merangkai skrip terminal Linux murni untuk System Administrator atau Red Teamer.
...
**5. Skenario Ruby/PHP: Server-Side Template Injection (SSTI)** _Pilih dropdown: Ruby atau PHP_ Skenario klasik eksploitasi web yang berujung pada Remote Code Execution (RCE).
...
```bash
npm install
Configure the environment variables by creating a .env file in the root directory and securely adding your API key:
